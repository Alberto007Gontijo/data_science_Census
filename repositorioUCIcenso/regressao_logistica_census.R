...
classificador = glm(formula = as.factor(income)~., family = binomial, data = base_treinamento)
classificador
previsoes = predict(classificador, type = 'response', newdata = base_teste)
previsoes
#conversao
probabilidades = ifelse(previsoes > 0.5, 1, 0)
probabilidades
matriz_confusao = table(base_teste[, 15], probabilidades)
library(caret)
